...
export const AuthUIProvider = ({
  ...
  const organization = useMemo<OrganizationOptionsContext | undefined>(() => {
    ...
    // Remove trailing slash from basePath
    const basePath = organizationProp.basePath?.endsWith("/") ? organizationProp.basePath.slice(0, -1) : organizationProp.basePath;
  ...
  const defaultHooks = useMemo(() => {
    return {
      ...
      useListInvitations: (params) =>
        useAuthData({
          queryFn: () => authClient.$fetch(`/organization/list-invitations?organizationId=${params?.query?.organizationId || ""}`),
      ...
      useListMembers: (params) =>
        useAuthData({
          queryFn: () => authClient.$fetch(`/organization/list-members?organizationId=${params?.query?.organizationId || ""}`),
          cacheKey: `listMembers:${JSON.stringify(params)}`,
        }),
      useListTeams: (params) =>
        useAuthData({
          queryFn: () => authClient.$fetch(`/organization/list-teams?organizationId=${params?.organizationId || ""}`),
        ...
        useAuthData({
          queryFn: () =>
            authClient.$fetch("/organization/list-team-members", {
              method: "POST",
              body: params?.teamId ? { query: { teamId: params.teamId } } : undefined,
            }),
  ...
  return (
    ...
    >
      ...
      {captcha?.provider === "google-recaptcha-v3" ? <RecaptchaV3>{children}</RecaptchaV3> : children}
    </AuthUIContext.Provider>
  );
};
