...
export function useSession({ triplit, sessionData, isPending, refetch, usePlural, modelNames }: UseTriplitOptionsProps): ReturnType<AuthHooks["useSession"]> {
  ...
  const { result: user, error } = useConditionalQueryOne(triplit, payload?.sub && triplit.query(modelName));
