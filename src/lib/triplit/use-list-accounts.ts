...
export function useListAccounts({ triplit, modelNames, usePlural, isPending }: UseTriplitOptionsProps): ReturnType<AuthHooks["useListAccounts"]> {
  ...
  const { results, error, fetching } = useConditionalQuery(triplit, payload?.sub && triplit.query(modelName));
