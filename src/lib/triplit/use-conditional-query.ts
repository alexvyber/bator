import type { FetchResult, Models, SchemaQuery, SubscriptionOptions, SubscriptionSignalPayload, TriplitClient } from "@triplit/client";
...
type useConditionalQueryOnePayload<M extends Models<M>, Q extends SchemaQuery<M>> = Omit<SubscriptionSignalPayload<M, Q>, "results"> & {
...
export function useConditionalQueryOne<M extends Models<M>, Q extends SchemaQuery<M>>(
  ...
  const { fetching, fetchingLocal, fetchingRemote, results, error } = useConditionalQuery(client, query ? ({ ...query, limit: 1 } as Q) : query, options);
