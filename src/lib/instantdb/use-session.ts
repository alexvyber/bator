...
export function useSession({ db, sessionData, isPending, refetch, usePlural, modelNames }: UseInstantOptionsProps): ReturnType<AuthHooks["useSession"]> {
  ...
  const { data } = db.useQuery(authUser ? { [modelName]: { $: { where: { id: authUser?.id } } } } : null);
