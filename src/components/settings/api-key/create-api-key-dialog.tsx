...
export function CreateApiKeyDialog({ classNames, localization, onSuccess, refetch, organizationId, onOpenChange, ...props }: CreateApiKeyDialogProps) {
  const {
    authClient,
    apiKey,
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      const expiresIn = values.expiresInDays && values.expiresInDays !== "none" ? Number.parseInt(values.expiresInDays) * 60 * 60 * 24 : undefined;

      const selectedOrgId = values.organizationId === "personal" ? undefined : values.organizationId;
    ...
    <Dialog onOpenChange={onOpenChange} {...props}>
      <DialogContent onOpenAutoFocus={(e) => e.preventDefault()} className={classNames?.dialog?.content}>
        <DialogHeader className={classNames?.dialog?.header}>
          ...
          <DialogDescription className={cn("text-xs md:text-sm", classNames?.description)}>{localization.CREATE_API_KEY_DESCRIPTION}</DialogDescription>
        </DialogHeader>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="w-full">
                    <FormLabel className={classNames?.label}>{localization.ORGANIZATION}</FormLabel>

                    <Select onValueChange={field.onChange} value={field.value} disabled={isSubmitting}>
                      <FormControl>
                        <SelectTrigger className={cn("w-full p-2", classNames?.input)}>
                          <SelectValue placeholder={localization.ORGANIZATION} />
                        </SelectTrigger>
                      </FormControl>

                      <SelectContent className="w-[--radix-select-trigger-width]">
                        <SelectItem value="personal" className="p-2">
                          <PersonalAccountView user={user} localization={localization} size="sm" />
                        </SelectItem>

                        {organizations?.map((org) => (
                          <SelectItem key={org.id} value={org.id} className="p-2">
                            <OrganizationCellView organization={org} localization={localization} size="sm" />
                          </SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="flex-1">
                    ...
                    <FormControl>
                      <Input className={classNames?.input} placeholder={localization.API_KEY_NAME_PLACEHOLDER} autoFocus disabled={isSubmitting} {...field} />
                    </FormControl>
            ...
            <DialogFooter className={classNames?.dialog?.footer}>
              ...
              <Button type="submit" variant="default" className={cn(classNames?.button, classNames?.primaryButton)} disabled={isSubmitting}>
