    ...
    >
      {deviceSessions?.length ? (
        <CardContent className={cn("grid gap-4", classNames?.content)}>
          {sessionData && <AccountCell classNames={classNames} deviceSession={sessionData} localization={localization} refetch={refetch} />}

          {otherDeviceSessions.map((deviceSession) => (
            <AccountCell key={deviceSession.session.id} classNames={classNames} deviceSession={deviceSession} localization={localization} refetch={refetch} />
          ))}
        </CardContent>
      ) : null}
    </SettingsCard>
