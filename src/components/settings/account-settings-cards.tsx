...
export function AccountSettingsCards({
  ...
  return (
    <div className={cn("flex w-full flex-col gap-4 md:gap-6", className, classNames?.cards)}>
      {accountOptions?.fields?.includes("image") && avatar && <UpdateAvatarCard classNames={classNames?.card} localization={localization} />}

      {credentials?.username && <UpdateUsernameCard classNames={classNames?.card} localization={localization} />}

      {accountOptions?.fields?.includes("name") && <UpdateNameCard classNames={classNames?.card} localization={localization} />}
