...
export function SecuritySettingsCards({
  ...
  return (
    <div className={cn("flex w-full flex-col gap-4 md:gap-6", className, classNames?.cards)}>
      {credentials && <ChangePasswordCard accounts={accounts} classNames={classNames?.card} isPending={accountsPending} localization={localization} skipHook />}
      ...
      {deleteUser && <DeleteAccountCard accounts={accounts} classNames={classNames?.card} isPending={accountsPending} localization={localization} skipHook />}
    </div>
  );
}
