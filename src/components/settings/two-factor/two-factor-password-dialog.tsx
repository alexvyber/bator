...
export function TwoFactorPasswordDialog({ classNames, onOpenChange, isTwoFactorEnabled, ...props }: TwoFactorPasswordDialogProps) {
  const { localization, authClient, basePath, viewPaths, navigate, toast, twoFactor, localizeErrors } = useContext(AuthUIContext);
    ...
    <>
      <Dialog onOpenChange={onOpenChange} {...props}>
        <DialogContent className={cn("sm:max-w-md", classNames?.dialog)}>
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(isTwoFactorEnabled ? disableTwoFactor : enableTwoFactor)} className="grid gap-4">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <PasswordInput className={classNames?.input} placeholder={localization.PASSWORD_PLACEHOLDER} autoComplete="current-password" {...field} />
                    </FormControl>
              ...
              <DialogFooter className={classNames?.dialog?.footer}>
                <Button type="button" variant="secondary" onClick={() => onOpenChange?.(false)} className={cn(classNames?.button, classNames?.secondaryButton)}>
                  {localization.CANCEL}
                </Button>
