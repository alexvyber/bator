      ...
      <Tailwind>
        <Body className={cn("mx-auto my-auto px-2 font-sans", classNames?.body)}>
          <Container className="mx-auto my-[40px] max-w-[465px] rounded border border-color border-solid p-[20px]">
            ...
            <Heading className={cn("mx-0 my-[30px] p-0 text-center font-bold text-[24px]", classNames?.heading)}>{heading}</Heading>

            <Text className={cn("text-[14px] leading-[24px]", classNames?.content)}>{content}</Text>

            {action && url && (
              <Section className="mt-[32px] mb-[32px] text-center">
                <Button className={cn("action-button rounded px-5 py-3 text-center font-semibold text-[12px] no-underline", classNames?.button)} href={url}>
                  {action}
                </Button>
              </Section>
            )}
