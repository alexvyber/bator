...
export function SignInForm({ className, classNames, isSubmitting, localization, redirectTo, setIsSubmitting, passwordValidation }: SignInFormProps) {
  ...
  async function signIn({ email, password, rememberMe }: z.infer<typeof formSchema>) {
    try {
      ...
      if (emailVerification?.otp && (error as { error?: { code?: string; message?: string } })?.error?.code === "EMAIL_NOT_VERIFIED") {
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(signIn)} noValidate={isHydrated} className={cn("grid w-full gap-6", className, classNames?.base)}>
        <FormField
          control={form.control}
          name="email"
          render={({ field }) => (
            <FormItem>
              <FormLabel className={classNames?.label}>{usernameEnabled ? localization.USERNAME : localization.EMAIL}</FormLabel>
