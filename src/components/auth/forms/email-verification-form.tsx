...
export function EmailVerificationForm({
  ...
  const { authClient, localization: contextLocalization, toast, localizeErrors, navigate, basePath, viewPaths } = useContext(AuthUIContext);
    ...
    return (
      <div className={cn("grid w-full gap-6", className)}>
        <div className="text-center">
          ...
          <p className="text-muted-foreground text-sm">{localization.EMAIL_REQUIRED || "Email address is required"}</p>
        </div>
      </div>
    );
  }
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(verifyCode)} className={cn("grid w-full gap-6", className, classNames?.base)}>
        ...
        <div className="grid gap-4">
          <Button type="submit" disabled={currentIsSubmitting} className={cn(classNames?.button, classNames?.primaryButton)}>
          ...
          >
            {resendDisabled ? `${localization.RESEND_VERIFICATION_EMAIL} (${countdown}s)` : localization.RESEND_VERIFICATION_EMAIL}
          </Button>

          {onCancel && (
            <Button type="button" variant="ghost" onClick={onCancel} disabled={currentIsSubmitting} className="w-full">
              {localization.CANCEL}
            </Button>
          )}
        </div>
