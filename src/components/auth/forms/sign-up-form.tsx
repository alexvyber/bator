...
export function SignUpForm({
  ...
  if (signUpFields) {
    for (const field of signUpFields) {
      ...
      if (additionalField.type === "number") {
        ...
        fieldSchema = additionalField.required ? z.string().min(1, `${additionalField.label} ${localization.IS_REQUIRED}`) : z.string().optional();
      }
  ...
  async function signUp({ email, password, name, username, confirmPassword, image, ...additionalFieldValues }: z.infer<typeof formSchema>) {
    try {
      ...
      if ("token" in data && data.token) {
        ...
        navigate(`${basePath}/${viewPaths.EMAIL_VERIFICATION}?email=${encodeURIComponent(email as string)}`);
      } else {
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(signUp)} noValidate={isHydrated} className={cn("grid w-full gap-6", className, classNames?.base)}>
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel className={classNames?.label}>
                  {localization.NAME}
                  {!nameRequired && <span className="ml-1 text-muted-foreground">{localization.OPTIONAL_BRACKETS}</span>}
                </FormLabel>
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel className={classNames?.label}>
                  ...
                  {!usernameRequired && <span className="ml-1 text-muted-foreground">{localization.OPTIONAL_BRACKETS}</span>}
                </FormLabel>
              ...
              <FormField
                ...
                render={({ field: formField }) => (
                  <FormItem className="flex">
                    <FormControl>
                      <Checkbox checked={formField.value as boolean} onCheckedChange={formField.onChange} disabled={isSubmitting} />
                    </FormControl>
        ...
        <Button type="submit" disabled={isSubmitting} className={cn("w-full", classNames?.button, classNames?.primaryButton)}>
