...
export function CreateOrganizationDialog({ className, classNames, localization: localizationProp, onOpenChange, ...props }: CreateOrganizationDialogProps) {
  const { authClient, localization: contextLocalization, organization: organizationOptions, navigate, toast, localizeErrors } = useContext(AuthUIContext);
    ...
    <Dialog onOpenChange={onOpenChange} {...props}>
      <DialogContent className={classNames?.dialog?.content}>
        <DialogHeader className={classNames?.dialog?.header}>
          ...
          <DialogDescription className={cn("text-xs md:text-sm", classNames?.description)}>{localization.ORGANIZATIONS_INSTRUCTIONS}</DialogDescription>
        </DialogHeader>
                    ...
                    <div className="flex items-center gap-4">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="start" onCloseAutoFocus={(e) => e.preventDefault()}>
                          ...
                          {logo && (
                            <DropdownMenuItem onClick={deleteLogo} disabled={logoPending} variant="destructive">
                              <Trash2Icon />
                      ...
                      <Button disabled={logoPending} variant="outline" onClick={openFileDialog} type="button">
