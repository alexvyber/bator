...
export function OrganizationLogoCard({ className, classNames, localization: localizationProp, slug, ...props }: OrganizationLogoCardProps) {
      ...
      <Card className={cn("w-full pb-0 text-start", className, classNames?.base)} {...props}>
        <div className="flex justify-between">
          ...
          <Button type="button" className="me-6 size-fit rounded-full" size="icon" variant="ghost" disabled>
            <OrganizationLogo isPending className="size-20 text-2xl" classNames={classNames?.avatar} localization={localization} />
          </Button>
        </div>

        <SettingsCardFooter className="!py-5" instructions={localization.LOGO_INSTRUCTIONS} classNames={classNames} isPending />
      </Card>
    );
  }

  return <OrganizationLogoForm className={className} classNames={classNames} localization={localization} organization={organization} {...props} />;
}
...
function OrganizationLogoForm({
  ...
  const localization = useMemo(() => ({ ...authLocalization, ...localizationProp }), [authLocalization, localizationProp]);
