 ...
 */
export function TeamCell({ className, classNames, team, localization, canDelete, canUpdate, refetch }: TeamCellProps) {
  /*
      ...
      <Card className={cn("flex-row items-center gap-3 truncate px-4 py-3", className, classNames?.cell)}>
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button className={cn("relative ms-auto", classNames?.button, classNames?.outlineButton)} size="icon" type="button" variant="outline">
              <EllipsisIcon className={classNames?.icon} />
            </Button>
          </DropdownMenuTrigger>

          <DropdownMenuContent onCloseAutoFocus={(e) => e.preventDefault()}>
            <DropdownMenuItem disabled={!canUpdate} onSelect={() => setShowUpdateDialog(true)}>
              <Edit className={classNames?.icon} />
              <span> {localization?.UPDATE_TEAM}</span>
            </DropdownMenuItem>

            <DropdownMenuItem disabled={!canDelete} onClick={() => setShowDeleteDialog(true)} variant="destructive">
