...
export function OrganizationSettingsCards({ className, classNames, localization, slug }: OrganizationSettingsCardsProps) {
  ...
  return (
    <div className={cn("flex w-full flex-col gap-4 md:gap-6", className, classNames?.cards)}>
      {organizationOptions?.logo && <OrganizationLogoCard classNames={classNames?.card} localization={localization} slug={slug} />}
