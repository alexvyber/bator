...
export function OrganizationSwitcher({
  ...
  }, [hidePersonal, activeOrganization, activeOrganizationPending, organizations, sessionPending, organizationPending, switchOrganization, slug]);
    ...
    <>
      <DropdownMenu open={dropdownOpen} onOpenChange={setDropdownOpen}>
        <DropdownMenuTrigger asChild>
          {trigger ||
            (size === "icon" ? (
              <Button size="icon" className={cn("size-fit rounded-full", className, classNames?.trigger?.base)} variant="ghost" type="button" {...props}>
              ...
              <Button className={cn("!p-2 h-fit", className, classNames?.trigger?.base)} size={size} {...props}>
                {isPending || activeOrganization || !sessionData || (user as User)?.isAnonymous || hidePersonal ? (
                  ...
                  <PersonalAccountView classNames={classNames?.trigger?.user} localization={localization} size={size} user={user} />
                )}
            ...
            {(user && !(user as User).isAnonymous) || isPending ? (
              <>
                ...
                {!isPending && (
                  ...
                  >
                    <Button size="icon" variant="outline" className="!size-8 ml-auto" onClick={() => setDropdownOpen(false)}>
