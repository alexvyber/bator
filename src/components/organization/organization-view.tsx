...
export function OrganizationView({
  ...
  }, [organization, organizationPending, organizationRefetching, accountOptions?.basePath, accountOptions?.viewPaths?.ORGANIZATIONS, replace]);
  ...
  return (
    <div className={cn("flex w-full grow flex-col gap-4 md:flex-row md:gap-12", className, classNames?.base)}>
      ...
      {view === "TEAMS" && organization?.id && teamsEnabled && (
        <TeamsCard classNames={classNames} localization={localization} organizationId={organization.id} />
      )}

      {view === "API_KEYS" && (
        <ApiKeysCard classNames={classNames?.card} localization={localization} isPending={organizationPending} organizationId={organization?.id} />
      )}

      {view === "SETTINGS" && <OrganizationSettingsCards classNames={classNames} localization={localization} slug={slug} />}
    </div>
  );
}
